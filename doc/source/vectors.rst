...
Vectors are the foundational ``planar2`` objects. They are used to
represent 2D vectors and geometric points. 

:class:`planar2.Vec2` objects are two dimensional, double precision
floating-point vectors. They can be initialized from either cartesian
or polar coordinates::

	>>> from planar2 import Vec2
	>>> v = Vec2(0, 1)
	>>> v.x
	0.0
	>>> v.y
...
:class:`~planar2.Seq2` and :class:`~planar.Vec2Array`.

:class:`~planar2.Seq2` is a mutable, but fixed-length, sequence of vectors.  It
...
:class:`~planar2.Vec2Array` is a full-featured vector sequence which provides
efficient storage and batch-operations for arbitrarily large collections of
:class:`~planar2.Vec2` objects. It is a subclass of :class:`~planar2.Seq2`.
...
on the entire collection of vectors efficiently. :class:`~planar2.Vec2Array`
objects are different from Python lists, however, in the following
important ways:

- Vector arrays can contain only :class:`~planar2.Vec2` objects.
...
:class:`~planar2.Vec2` objects::

	>>> from planar2 import Vec2Array
	>>> a = Vec2Array()
	>>> len(a)
	0
...
therein::
	...
	>>> from planar2 import Vec2Array, Affine
...
:class:`~planar2.Vec2Array` class reference for details.
