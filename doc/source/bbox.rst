...
type in ``planar2``, represented by two points containing the minimum and
...
``planar2`` have a ``bounding_box`` attribute which returns the smallest
...
points::
	...
	>>> from planar2 import BoundingBox
...
The resulting bounding box encloses all of the shapes provided::
	...
	>>> from planar2 import BoundingBox, Polygon
	>>> shapes = [
...
:meth:`~planar2.BoundingBox.contains_point()` method for testing if a point is
...
near the shape at all. This is done automatically by built-in ``planar2``
shapes where appropriate, but you can employ this strategy yourself if you
create your own custom shape classes with ``planar2`` primitives.

:meth:`~planar2.BoundingBox.contains_point()` has special semantics for
...
edges. Since neither of these results is satisfactory, ``planar2`` instead
...
:class:`~planar2.BoundingBox` instance when transformed. Such transforms
include translation, scale, and rotation by multiples of 90 degrees::

	>>> from planar2 import BoundingBox, Affine
...
but the result will be a :class:`~planar2.Polygon` since the resulting shape
...
:class:`~planar2.Polygon`. Since polygons are mutable, this is one way to get
around the immutability of :class:`~planar2.BoundingBox` instances. This can
...
amount using the :meth:`~planar2.BoundingBox.inflate()` method. This method
...
:meth:`~planar2.BoundingBox.inflate()` to change the width and height by the
...
providing a sequence of two numbers, such as a tuple, or :class:`~planar2.Vec2`
object::
...
:class:`~planar2.BoundingBox` :meth:`~planar2.BoundingBox.fit()` method takes an
